...
it('knight move from center', () => {
  const D5 = testUtil.posArrayToNotationArray(chess.allValidMoves("D5", state))   
  expect(D5).toStrictEqual(["C7", "E7", "F6", "F4", "E3", "C3", "B4", "B6"].sort())
})

it('knight move from top left corner', () => {
  const A8 = testUtil.posArrayToNotationArray(chess.allValidMoves("A8", state))   
...
it('knight move from top left corner', () => {
  const F7 = testUtil.posArrayToNotationArray(chess.allValidMoves("F7", state))   
